...
public static class SimilarityScorer
{
    ...
    /// 実測（本環境）ではこの帯DPは毎秒約2.5億セルを処理するため、2億セルは最悪でも
    /// 0.80秒に相当する（予算を使い切る病的な入力で実測0.80秒）。
    /// 枝刈りが効く通常の入力ではここに遠く届かない。予算を二分探索して求めた実測では、
    /// 10万行×SEARCH100行が打ち切られずに済む最小予算は **207,159セル**（この上限の約0.1%）だった。
    /// つまり1000倍近い余裕がある。届くのは「どの窓もSEARCH部の並べ替えになっている」ような
    /// 病的な場合だけで、そのときは何分も待たせるより
    /// 「大きすぎて照合しきれませんでした」と理由を返す方がよい。
    /// </summary>
