...
public enum ErrorCode
{
    ...
    /// <summary>
    /// 実機不具合対応: SEARCH/REPLACE対象のファイルが見つからない、またはこの環境から
    /// 読み取れない（16章の表には無いため追加）。ネットワークドライブ上でファイルの存在確認
    /// 自体が誤って失敗する不具合（LongPath.Extended参照）が実機で見つかり、その結果
    /// DryRunPlanner.LoadCurrentLinesAsyncが空の行リストを返して、E101（SEARCH部が見つからない）
    /// と誤表示されていた。E101は「ファイルは読めたが中身が一致しない」という意味に読めるため、
    /// 「そもそもファイルを読めていない」場合と区別する。FULL形式（新規作成が正規の用途）や
