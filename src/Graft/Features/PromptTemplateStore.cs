...
public sealed class PromptTemplateStore
{
    ...
    ///
    /// 【v1.0.20: フェンス開始行の字下げ禁止を追記した経緯】<see cref="CodeBlockWrapNote"/> のコメントを
    /// 参照（利用者の実機で起きた事故の調査に基づく予防的な指示）。v1.0.13以降、既定テンプレートは
    /// Graft独自形式ではなくこちらの標準SR形式を使うため、この追記はむしろこちらでこそ重要になる。
    /// </summary>
    private const string StandardCodeBlockWrapNote =
        ...
        "```text の行は必ず行頭（列0）から書き、字下げしないでください" +
        "（字下げすると、Markdownの仕様上その分だけ中身の各行からも空白が取り除かれ、" +
        "インデントが崩れた状態でコピーされます）。";
    ...
    ///
    /// 【v1.0.20: フェンス開始行の字下げ禁止を追記した経緯】利用者の実機で、MODE=FULLで新規作成した
    /// 複数ファイルが、インデントのある行だけ行頭空白が1個ずつ減った状態でディスクに書き込まれる
    /// 事故が報告された（パッチ本文では `  to: "",`（2個）・`    "宛先：..."`（4個）だったものが、
    /// 実ファイルでは ` to: "",`（1個）・`   "宛先：..."`（3個）になっていた。列0の行——コメントや
    /// const、閉じ括弧、`&lt;&lt;&lt;&lt; FILE:` 等のマーカー行——は無傷で、インデントを持つ行だけが
    /// 1文字減っていた）。パーサ・書き込み・コンテキスト収集の各段階を実測で確認したが、Graft側の
    /// 処理はすべてバイト単位で保全されており、利用者が実際に貼り付けたパッチをそのまま通しても
    /// 完全一致した。一方、CommonMarkの仕様では、フェンス付きコードブロックの開始行がNスペース
    /// 字下げされていると、中身の各行から先頭最大Nスペースが取り除かれると定められており、これは
    /// 開始行が1スペース字下げされた場合に観測された壊れ方（列0の行は無傷、2スペースの行は
    /// 1スペースへ、4スペースの行は3スペースへ）と一致する。チャットUIの「コピー」ボタンは
    /// 解釈済みの中身を返すため、この目減りはそのままクリップボードへ渡り、Graftに届く時点で
    /// 既に失われており、Graft側からは検出しようがない。原因を断定できる証拠ではないが、他の
    /// どの説明よりも証拠の形に合致するため、AIの出力段階でフェンス開始行を列0から書かせることで
    /// 発生源そのものを塞ぐ、予防的な指示をここに追記した。
    /// </summary>
