...
/// 課題3の極端に長い行の通知・10件目の不具合修正のグローバルホットキー再登録失敗）を
/// 1箇所へ集約する。
///
...
public sealed partial class ShellViewModel
{
    ...
    private (Func<bool> IsActive, string Text, string Detail)[] BuildStatusBarWarningSources() => new (Func<bool>, string, string)[]
    {
        ...
        (
            ...
            "極端に長い行があります（その行のみ構文強調を簡略化）",
            "この行は20,000文字を超えるため、構文強調・括弧の対応付けはその行に限って簡略化しています"
                + "（他の行は通常どおりです）。折り返し・コードの折りたたみは設定どおり有効です。"
                + "折り返しが有効な場合、この行の表示が重くなることがあります。気になる場合は"
                + "エディタ上部の通知バーから、このファイルだけ折り返しを無効にできます。"
        ),
    };
