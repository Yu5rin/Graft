...
/// <see cref="WindowsTrayIcon"/>（登録・メッセージ処理）と
/// <see cref="WindowsTrayIconRenderer"/>（HICON生成）の双方から参照される。
/// <see cref="System.Windows.Forms.NotifyIcon"/> はWinForms参照を要し配布条件（附録A.3）に
/// 反するため使わず、<c>Shell_NotifyIcon</c> を直接P/Invokeする方針の一部。
/// 移設元は <c>Views/TrayNativeMethods.cs</c>。宣言内容は変更しない。
/// </summary>
[SupportedOSPlatform("windows")]
internal static class WindowsTrayNativeMethods
{
