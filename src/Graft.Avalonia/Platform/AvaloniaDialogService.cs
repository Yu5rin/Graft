...
/// <summary>
/// <see cref="IDialogService"/> のAvalonia実装。WPF版（<c>Graft.Views.DialogService</c>）と
...
/// シグネチャへ変更した（WPF版は<see cref="Task.FromResult{TResult}(TResult)"/>で包むだけで
/// 追随できる）。
///
/// もう一つの差異は<see cref="Window.ShowDialog(Window)"/>がオーナーウィンドウを必須とする点
/// （WPF版はオーナー無しの<c>ShowDialog()</c>を持つ）。<see cref="FindOwnerWindow"/>で
