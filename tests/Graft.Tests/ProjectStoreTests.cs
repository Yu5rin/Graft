...
public class ProjectStoreTests
{
    ...
    [Fact(DisplayName = "RegisterAsyncはデータ保存先そのものの登録をE215で拒否する")]
    public async Task RegisterAsyncはデータ保存先自身を拒否する()
    {
        ...
        result.IsSuccess.Should().BeFalse();
        result.Errors.Single().Code.Should().Be(ErrorCode.E215);
    }

    [Fact(DisplayName = "RegisterAsyncはデータ保存先を含む親フォルダの登録をE215で拒否する")]
    ...
    [Fact(DisplayName = "RelocateAsyncはデータ保存先と重なる場所への移動をE215で拒否する")]
    public async Task RelocateAsyncはデータ保存先と重なる移動先を拒否する()
    {
        ...
        relocated.Errors.Single().Code.Should().Be(ErrorCode.E215);
