...
///
/// 【誤検知の修正】取扱説明書のようにパッチの書き方をコードブロックで例示している
/// だけの文書を、内容をコピーしただけで検知してしまう不具合の回帰テストを含む
/// （詳細は<see cref="PatchTextDetector"/>のクラスコメント参照）。判定は
/// 「マーカーがコードブロックの外にあり、かつ対応関係（PatchParserで構造として
/// 成立しているか）が揃っている」ことを条件とする。ただし見逃し（本物のパッチを
/// 検知しない）の方が誤検知より害が大きいため、パスが不正・SEARCHが空といった
/// 「内容の誤り」で構造そのものは認識できているケースまでは非検知にしない
...
public class PatchTextDetectorTests
{
    ...
    [Fact(DisplayName = "見逃し防止_単一フェンスで丸ごと囲まれたパッチはクリップボード自動検知の対象外（手動解析では従来どおり成功する）")]
    public void 単一フェンスで丸ごと囲まれたパッチは自動検知しないが手動解析はできる()
    {
        // markdown_fence.txt: 「以下がパッチです。」+ ```で丸ごと囲まれたパッチ + 「以上です。」
        // という、AIがチャットの表示上パッチ全体をコードブロックで囲んだ場合の実例。
        // マーカーがすべてコードブロックの内側にあるため、今回の方針（コードブロック外の
        // マーカーだけを見る）では自動検知の対象にはならない、という意図的なトレードオフ。
        // ただし手動の「解析」ボタン（PatchParser.Parse直接呼び出し）は影響を受けず、
        // 既存のPatchParserTests「Markdownコードフェンスに囲まれたパッチも解析できる」が
        // 示すとおり従来どおり成功する。
        var text = FixtureLoader.LoadPatch("markdown_fence");

        PatchTextDetector.LooksLikePatch(text).Should().BeFalse(
            "マーカーがコードブロックの外に無いため、今回の方針では自動検知の対象にしない");

        var act = () => new PatchParser().Parse(text);
        act.Should().NotThrow();
        act().IsSuccess.Should().BeTrue("手動の「解析」は従来どおり成功するはず（自動検知とは別経路）");
    }
