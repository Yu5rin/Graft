...
public class PathGuardTests
{
    ...
    [Fact(DisplayName = "ProtectedDataDirectory自身への書き込みはE215で拒否される（プロジェクトルート＝データ保存先のケース）")]
    public void データ保存先自身への書き込みはE215で拒否される()
    {
        using var ws = new TempWorkspace();
    ...
    [Fact(DisplayName = "ProtectedDataDirectory配下（サブフォルダ）への書き込みもE215で拒否される")]
    public void データ保存先の配下への書き込みもE215で拒否される()
    {
        using var ws = new TempWorkspace();
