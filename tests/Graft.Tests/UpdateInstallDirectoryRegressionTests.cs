...
public class UpdateInstallDirectoryRegressionTests
{
    ...
    public async Task 不具合再現_データ保存先を渡すと失敗する()
    {
        ...
        var result = await pipeline.RunAsync(
            asset, userDataDir, ws.Combine("work-buggy"), checkUrl: "https://example.invalid/releases/latest",
    ...
    public async Task 修正後_実行ファイルのフォルダを渡すと成功する()
    {
        ...
        var result = await pipeline.RunAsync(
            asset, installDirectory!, ws.Combine("work-fixed"), checkUrl: "https://example.invalid/releases/latest",
            allowMissingChecksum: false, downloadProgress: null, CancellationToken.None);
