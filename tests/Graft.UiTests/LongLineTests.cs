...
public class LongLineTests : IDisposable
{
    ...
    ///
    /// 【CI 714/715 不定期失敗の再調査（本コミット、閾値そのものを見直し）】
    /// 上記の頑健化（組数7→15・トリム済み中央値の採用）は「1〜2組だけの外れ値」には効くが、
    /// 今回GitHub Actions上で実際に落ちたケースは性質が異なった: 15組中11組（トリム対象の
    /// 上下2組ずつを除いた残り）が軒並み倍率3.2〜4倍に収まっており、外れ値ではなく
    /// 分布そのものが3.0付近から底上げされていた（トリム済み中央値3.54倍、実測ログ:
    /// 基準[9.973, 115.624, 10.597, 8.151, 8.682, 8.115, 10.663, 8.756, 9.312, 9.567, 11.743,
    /// 38.079, 11.306, 8.859, 9.036]ms・対象[35.283, 26.597, 35.096, 62.285, 32.721, 28.148,
    /// 34.225, 34.611, 34.422, 33.646, 65.848, 27.354, 31.497, 31.909, 31.981]ms）。統計処理を
    /// いくら頑健にしても、外れ値ではなく中央値そのものが動く系統的なズレは吸収できない。
    ///
    /// 原因はキャップ機構の劣化ではなく、「1行10万文字」と「総文字数を揃えた1250行×80文字」の
    /// 描画コストの比が、実行環境（CPU・Skiaのテキストシェイピング実装・キャッシュ特性等）に
    /// よって数割単位で変動すること自体にある。負荷をかけた状態・taskset -c 0,1（2コア）で
    /// このリポジトリのCI環境を模して合計30回以上ローカル再実行したが、倍率は0.59〜5.01倍の
    /// 範囲に収まり（トリム済み中央値は常に1.3〜2.0倍程度）、3.54倍という値そのものは
    /// このマシンでは再現できなかった。しかし観測された分布の形（外れ値ではなく中央値の底上げ）
    /// は「環境差」の特徴と一致し、「その行だけキャップする」経路が壊れて初めて起きる劣化とは
    /// 桁が違う（クラスコメントの実測: キャップが効かないと n=100,000 で旧方式は約80秒、
    /// キャップ後は約4.3ms＝比にして数万倍。数倍程度の揺れとは症状の桁が全く異なる）。
    ///
    /// そのため、閾値を3.0→6.0へ引き上げる。これは「落ちたから緩める」対症療法ではなく、
    /// 上記の実測（環境差は最大でも5倍程度に収まる一方、本物の regression は数万倍の桁で
    /// 現れる）に基づき、検出力を実質的に犠牲にせず環境差だけを吸収する値として選んでいる。
    /// </summary>
    private const double RatioThreshold = 6.0;
